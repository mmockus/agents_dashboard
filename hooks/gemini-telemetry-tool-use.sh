...
SESSION_ID="${GEMINI_SESSION_ID:-$(extract_json_value "$INPUT" "session_id")}"
...
# Prefer the description field when available (contains Gemini's task context)
case "$TOOL_NAME" in
  "Edit"|"Write")
...
# Get project name
PROJECT_NAME=$(basename "${GEMINI_CWD:-$(pwd)}")
